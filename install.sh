#!/bin/bash
#
# Proot Ubuntu Installer - Auto Install
...
show_header() {
    ...
    echo "#####################################################################################"
    echo "#                                                                                   #"
    echo "#                         Ubuntu Proot Auto Installer                               #"
    echo "#                                                                                   #"
...
if [ -d "$ROOTFS_DIR" ]; then
    ...
    chmod -R 777 "$ROOTFS_DIR" 2>/dev/null
    rm -rf "$ROOTFS_DIR"
...
if command -v proot &>/dev/null; then
    proot --link2symlink tar -xf "$TARBALL" -C "$ROOTFS_DIR" --exclude='dev' 2>/dev/null
else
...
echo -e "  ${CYAN}chmod -R 777 ${ROOTFS_DIR} && rm -rf ${ROOTFS_DIR} ${LAUNCH}${NC}"
echo ""
